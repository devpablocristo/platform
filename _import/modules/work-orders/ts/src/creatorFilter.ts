/**
 * Filtro por creador de OT — reutilizable en cualquier tablero kanban.
 */

export type CreatorFilterState = { mode: 'all' } | { mode: 'pick'; actors: Set<string> };

export function formatActorLabel(actor: string, selfId: string | undefined): string {
...
export function applyCreatorFilter<T extends { id: string; created_by?: string }>(
  rows: T[],
  opts: {
    authEnabled: boolean;
...
export function isYoFilterActive(creatorFilter: CreatorFilterState, selfId: string | undefined): boolean {
  return (

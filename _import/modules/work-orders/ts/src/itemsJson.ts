/**
 * Parser y serializer de ítems de OT en formato JSON (textarea).
 */
import type { WorkOrderItem } from './types';

export function parseWorkOrderItemsJson(text: string): WorkOrderItem[] {
  const t = text.trim();
  if (!t) return [];
  let raw: unknown;
  try {
...
export function stringifyWorkOrderItems(items: WorkOrderItem[] | undefined): string {

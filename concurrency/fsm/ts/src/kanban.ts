/**
 * Kanban transition model — a helper that pairs the FSM Builder with
 * column/status mappings for drag-and-drop kanban UIs. Domain-agnostic
 * (Status/ColumnId are generic string-literal types; the consumer supplies
 * the vocabulary).
 *
 * Typical use:
 *
 *   const model = createKanbanTransitionModel<MyStatus, MyColumnId>({
 *     normalizeStatus: s => s.trim().toLowerCase() as MyStatus,
 *     columns: [
 *       { columnId: 'todo', defaultStatus: 'open', statuses: ['open', 'pending'] },
 *       { columnId: 'done', defaultStatus: 'closed', statuses: ['closed'] },
 *     ],
 *     terminalStatuses: ['closed'],
 *     transitions: [
 *       { from: 'open',    to: ['pending', 'closed'] },
 *       { from: 'pending', to: ['closed'] },
 *     ],
 *   });
 *
 *   model.canMoveToColumn(row.status, 'done'); // boolean
 *   model.canTransitionToStatus(from, to);     // boolean
 */

import { Builder, type StringMachine } from "./fsm";

export type KanbanTransitionModel<
...
export function createKanbanTransitionModel<
...
>({
  ...
  terminalStatuses = [],
  transitions = [],
}: Options<Status, ColumnId>): KanbanTransitionModel<Status, ColumnId> {
  ...
  const getDefaultStatusForColumn = (columnId: ColumnId): Status | null =>
    defaultStatusByColumn.get(columnId) ?? null;

"""OpenTelemetry bootstrap for FastAPI + outbound httpx clients."""

from __future__ import annotations

from typing import Any

from runtime.logging import format_log_event, get_logger
...
def configure_opentelemetry(app: Any, settings: Any, backend_client: Any | None = None) -> None:
    ...
    try:
        ...
        logger.warning(format_log_event("otel_unavailable", error=str(exc)))
        return

    resource = Resource.create(
        {
            "service.name": getattr(settings, "otel_service_name", "core-ai"),
    ...
    logger.info(format_log_event("otel_configured", endpoint=endpoint or "console"))
